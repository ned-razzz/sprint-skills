...
def require_base_url() -> str:
    ...
    if not base_url:
        raise ConfigError("CONFLUENCE_BASE_URL must be set")
    parsed = urlparse(base_url)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise ConfigError("CONFLUENCE_BASE_URL must be a valid http(s) URL")
    return base_url
...
def ensure_heading_for_drawio_sections(markdown_body: str, title: str) -> str:
    if not markdown_body.strip():
        return markdown_body
    if re.search(r"^(#{1,6})[ \t]+.+$", markdown_body, re.MULTILINE):
        return markdown_body
    if "confluence-drawio" not in markdown_body.casefold():
        return markdown_body
    heading = title.strip() or "Diagram"
...
def process_title(
    ...
    markdown = ensure_heading_for_drawio_sections(markdown, title)
    version_number = int(((page.get("version") or {}).get("number")) or 0)
    source = build_page_url(client.base_url, page)
