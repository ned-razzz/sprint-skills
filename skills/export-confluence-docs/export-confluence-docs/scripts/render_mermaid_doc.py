...
def front_matter_title(front_matter: str) -> str | None:
...
def ensure_sectioned_body(front_matter: str, body: str) -> str:
    if parse_sections(body):
        return body
    lowered = body.casefold()
    if "confluence-drawio" not in lowered and MERMAID_FENCE not in lowered:
        return body
    title = front_matter_title(front_matter) or "Diagram"
...
def main() -> int:
    try:
        ...
        body = ensure_sectioned_body(front_matter, body)
