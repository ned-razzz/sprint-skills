...
class ExportConfluenceDocsTests(unittest.TestCase):
    ...
    def test_require_base_url_reads_environment(self) -> None:
        previous = os.environ.get("CONFLUENCE_BASE_URL")
        try:
            os.environ["CONFLUENCE_BASE_URL"] = "https://example.atlassian.net/wiki"
            self.assertEqual(bundle.require_base_url(), "https://example.atlassian.net/wiki")
        finally:
            if previous is None:
                os.environ.pop("CONFLUENCE_BASE_URL", None)
            else:
                os.environ["CONFLUENCE_BASE_URL"] = previous
    ...
    def test_export_bundle_adds_heading_for_placeholder_only_document(self) -> None:
        markdown = '<!-- confluence-drawio diagram="state" diagram_slug="state" owner_page_id="1" source="adf-extension" -->\n'
        rendered = bundle.ensure_heading_for_drawio_sections(markdown, "State Machine Diagram")
        self.assertTrue(rendered.startswith("# State Machine Diagram\n\n"))
    ...
    def test_render_mermaid_doc_adds_heading_for_placeholder_only_body(self) -> None:
        original = textwrap.dedent(
            """\
            ---
            title: "State Machine Diagram"
            confluence_page_id: "52166707"
            ---

            <!-- confluence-drawio diagram="state" diagram_slug="state" owner_page_id="52166707" source="adf-extension" -->
            """
        )
        front_matter, body = render_doc.split_front_matter(original)
        sectioned = render_doc.ensure_sectioned_body(front_matter, body)
        sections = render_doc.parse_sections(sectioned)
        self.assertEqual(sections[0]["heading"], "State Machine Diagram")
        self.assertIn("confluence-drawio", sections[0]["body"])
