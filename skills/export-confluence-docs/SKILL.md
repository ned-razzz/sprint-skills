...
7. Run `python3 scripts/map_doc_drawio.py --doc <markdown-path> --xml-dir <temp-xml-dir>` and save the mapping JSON.
8. Run `python3 scripts/render_drawio_mermaid.py --map-json <map-json> --out <diagram-json>` to build deterministic Mermaid from the mapped XML files.
9. Run `python3 scripts/render_mermaid_doc.py --doc <markdown-path> --diagram-json <diagram-json>` to replace the placeholder section body with Mermaid.
...
- The local scripts in this skill are for mapping, deterministic Mermaid generation, and Markdown rewrite after MCP has already provided the page content and attachment files.
